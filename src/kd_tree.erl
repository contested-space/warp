...
-record(node, {
    ...
    % TODO: specify a better type for value
...
insert({X, Y, Z} = Key, Value, Tree) when
    is_integer(X) andalso is_integer(Y) andalso is_integer(Z)
->
...
insert({X, Y, Z}, _Value, #node{key = {X, Y, Z}}, _CycleList) ->
    % TODO: maybe not crash when key exists?
    error(<<"Key already exists">>);
insert({X, _Y, _Z} = Key, Value, #node{key = {TreeX, _, _}, left_node = LeftNode} = Tree, [
    x | CycleList
]) when
    X < TreeX
->
...
insert({_X, Y, _Z} = Key, Value, #node{key = {_, TreeY, _}, left_node = LeftNode} = Tree, [
    y | CycleList
]) when
    Y < TreeY
->
...
insert(Key, Value, #node{right_node = RightNode} = Tree, [y | CycleList]) ->
    Tree#node{right_node = insert(Key, Value, RightNode, CycleList)};
insert({_X, _Y, Z} = Key, Value, #node{key = {_, _, TreeZ}, left_node = LeftNode} = Tree, [
    z | CycleList
]) when
    Z < TreeZ
->
...
lookup({X, _Y, _Z} = Key, #node{key = {TreeX, _, _}, left_node = LeftNode} = _Tree, [x | CycleList]) when
    X < TreeX
->
    lookup(Key, LeftNode, CycleList);
lookup(Key, #node{right_node = RightNode} = _Tree, [x | CycleList]) ->
    lookup(Key, RightNode, CycleList);
lookup({_X, Y, _Z} = Key, #node{key = {_, TreeY, _}, left_node = LeftNode} = _Tree, [y | CycleList]) when
    Y < TreeY
->
    lookup(Key, LeftNode, CycleList);
lookup(Key, #node{right_node = RightNode} = _Tree, [y | CycleList]) ->
    lookup(Key, RightNode, CycleList);
lookup({_X, _Y, Z} = Key, #node{key = {_, _, TreeZ}, left_node = LeftNode} = _Tree, [z | CycleList]) when
    Z < TreeZ
->
    lookup(Key, LeftNode, CycleList);
...
delete({X, Y, Z}, #node{key = {X, Y, Z}}, _CycleList) ->
    none;
delete({X, _Y, _Z} = Key, #node{key = {TreeX, _, _}, left_node = LeftNode} = Tree, [x | CycleList]) when
    X < TreeX
->
...
delete({_X, Y, _Z} = Key, #node{key = {_, TreeY, _}, left_node = LeftNode} = Tree, [y | CycleList]) when
    Y < TreeY
->
    Tree#node{left_node = delete(Key, LeftNode, CycleList)};
delete(Key, #node{right_node = RightNode} = Tree, [y | CycleList]) ->
    Tree#node{right_node = delete(Key, RightNode, CycleList)};
delete({_X, _Y, Z} = Key, #node{key = {_, _, TreeZ}, left_node = LeftNode} = Tree, [z | CycleList]) when
    Z < TreeZ
->
    Tree#node{left_node = delete(Key, LeftNode, CycleList)};
...
-spec fold(fun((key(), Value :: term(), FunAcc :: term()) -> term()), Acc :: term(), tree()) ->
    term().
% fold expects a 3 argument function that takes Key, Value and Acc, and returns an updated Acc.
fold(_Fun, Acc, none) ->
    Acc;
fold(
    Fun, Acc, #node{key = Key, value = Value, left_node = LeftNode, right_node = RightNode} = _Tree
) ->
...
get_sphere({X, Y, Z} = Key, Radius, Tree) when
...
get_cube(
    Key, Radius, #node{left_node = LeftNode, right_node = RightNode} = _Tree, [x | CycleList], Acc
) ->
...
get_cube(
    ...
    X =< TreeX + Radius andalso
        X >= TreeX - Radius andalso Z =< TreeZ + Radius andalso Z >= TreeZ - Radius
->
    Acc1 = get_cube(Key, Radius, LeftNode, CycleList, [{NodeKey, Value} | Acc]),
    get_cube(Key, Radius, RightNode, CycleList, Acc1);
get_cube(
    Key, Radius, #node{left_node = LeftNode, right_node = RightNode} = _Tree, [y | CycleList], Acc
) ->
    Acc1 = get_cube(Key, Radius, LeftNode, CycleList, Acc),
    get_cube(Key, Radius, RightNode, CycleList, Acc1);
get_cube(
...
->
    ...
    get_cube(Key, Radius, RightNode, CycleList, Acc1);
get_cube(
    Key, Radius, #node{left_node = LeftNode, right_node = RightNode} = _Tree, [z | CycleList], Acc
) ->
    Acc1 = get_cube(Key, Radius, LeftNode, CycleList, Acc),
    get_cube(Key, Radius, RightNode, CycleList, Acc1).
